...
class ProviderInfo {
    constructor(id, name, subnet_tag) {
        this.id = id;
        this.name = name;
...
class SummaryLogger {
    ...
    _reset() {
        ...
        this.task_data = {};
        this.provider_tasks = new Map();
        this.provider_cost = new Map();
        this.provider_failures = new Map();
        this.finished = false;
    ...
    _handle(event) {
        ...
        if (eventName === events.SubscriptionCreated.name)
            ...
            this.agreement_provider_info[event["agr_id"]] = new ProviderInfo(event["provider_id"], provider_name, event["provider_info"].subnet_tag);
        }
            ...
            logger.info(`Task sent to provider '${provider_info.name}', task data: ${event["task_id"]
            ...
            const provider_info = this.agreement_provider_info[event["agr_id"]];
            logger.info(`Task computed by provider '${provider_info.name}', task data: ${event["task_id"]
                ? this.task_data[event["task_id"]]
                : "<initialization>"}`);
            if (event["task_id"]) {
            ...
            logger.debug(`Received an invoice from ${provider_info.name}. Amount: ${event["amount"]}; (so far: ${cost} from this provider).`);
        }
            ...
            if (!failures)
                this.provider_failures.set(provider_info, 0);
            else
                this.provider_failures.set(provider_info, failures + 1);
            let more_info = "";
            if (event["exception"] &&
...
exports.changeLogLevel = (level) => {
    ...
    options.transports = [
        ...
        new winston_1.default.transports.File({
            filename: path_1.default.join("logs", `yajsapi-${dayjs_1.default().format("YYYY-MM-DD_HH-mm-ss")}.log`),
            level: "silly",
        }),
        new winston_1.default.transports.File({
            filename: path_1.default.join("logs", "yajsapi-current.log"),
            level: "silly",
            options: { flags: "w" },
        })
    ];
